        ...
        <section id="pricing" className="py-32 bg-white dark:bg-slate-950">
            <div className="container mx-auto px-6">
                <div className="mb-16 md:mb-24 text-center">
                    <p className="text-[10px] md:text-sm font-black uppercase tracking-[0.4rem] text-emerald-600 mb-6">Inversión Inteligente</p>
                    <h2 className="text-4xl font-black tracking-tighter text-slate-900 dark:text-white sm:text-6xl mb-8 md:mb-12">
                    ...
                    <div className="flex items-center justify-center gap-4">
                        <button
                            ...
                            className={`text-xs md:text-sm font-black uppercase tracking-widest ${!isYearly ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}
                        >
                            Mensual
                        </button>
                        <button
                            onClick={() => setIsYearly(!isYearly)}
                            className="relative h-8 md:h-10 w-16 md:w-20 rounded-full bg-slate-100 dark:bg-slate-800 p-1"
                        >
                            <motion.div
                                animate={{ x: isYearly ? (typeof window !== 'undefined' && window.innerWidth < 768 ? 32 : 40) : 0 }}
                                className="h-6 w-6 md:h-8 md:w-8 rounded-full bg-emerald-600"
                            />
                        </button>
                        <button
                            onClick={() => setIsYearly(true)}
                            className={`text-xs md:text-sm font-black uppercase tracking-widest flex items-center gap-2 ${isYearly ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}
                        >
                            Anual
                            <span className="rounded-full bg-emerald-100 dark:bg-emerald-500/10 px-2 py-1 text-[9px] md:text-[10px] text-emerald-600 font-black">-20%</span>
                        </button>
                    </div>
                </div>

                <div className="grid gap-6 md:gap-10 md:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
                    {plans.map((plan, i) => (
                        <motion.div
                            key={plan.name}
                            ...
                            className={`relative flex flex-col rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-12 transition-all hover:scale-[1.02] ${plan.popular ? 'bg-slate-900 dark:bg-emerald-900/10 border-2 border-emerald-500 text-white shadow-2xl shadow-emerald-500/20' : 'bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white'}`}
                        >
                            {plan.popular && (
                                <div className="absolute top-8 right-8 md:top-10 md:right-10 rounded-full bg-emerald-600 px-3 md:px-4 py-1.5 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white">
                                    Más Popular
                                </div>
                            )}

                            <h3 className="text-2xl md:text-3xl font-black mb-2 tracking-tight">{plan.name}</h3>
                            <p className={`text-base md:text-lg font-medium mb-8 md:mb-12 ${plan.popular ? 'text-slate-400' : 'text-slate-500'}`}>{plan.desc}</p>
                            ...
                            <div className="mb-8 md:mb-12 flex items-baseline gap-2">
                                ...
                                <span className={`text-base md:text-lg font-bold ${plan.popular ? 'opacity-60' : 'text-slate-400'}`}>/ {isYearly ? "año" : "mes"}</span>
                            </div>
