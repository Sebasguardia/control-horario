...
export const Hero = () => {
    return (
        <section className="relative pt-24 pb-16 md:pt-32 md:pb-20 lg:pt-56 lg:pb-32 overflow-hidden bg-white dark:bg-slate-950">
            {/* Background elements */}
            <div className="absolute top-[-10%] right-[-10%] h-[300px] w-[300px] md:h-[500px] md:w-[500px] rounded-full bg-emerald-500/10 blur-[80px] md:blur-[120px]" />
            <div className="absolute bottom-[-10%] left-[-10%] h-[400px] w-[400px] md:h-[600px] md:w-[600px] rounded-full bg-emerald-600/5 blur-[100px] md:blur-[150px]" />
                ...
                <div className="grid items-center gap-12 lg:gap-16 lg:grid-cols-2">
                    ...
                    >
                        <motion.div
                            ...
                            className="inline-flex items-center gap-2 rounded-full border border-emerald-100 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/10 px-4 py-2 mb-6 md:mb-8"
                        >
                            <Zap className="h-4 w-4 text-emerald-500 fill-emerald-500" />
                            <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">Control de tiempo de nueva generación</span>
                        </motion.div>

                        <h1 className="text-4xl xs:text-5xl font-black leading-[0.95] tracking-tighter text-slate-900 dark:text-white sm:text-7xl lg:text-[90px]">
                            Domina tu jornada <br />
                            <span className="text-emerald-600 italic">sin esfuerzo.</span>
                        </h1>

                        <p className="mt-6 md:mt-8 max-w-lg text-base md:text-lg font-medium leading-relaxed text-slate-500 dark:text-slate-400 md:text-xl">
                        ...
                        <div className="mt-10 md:mt-12 flex flex-col gap-4 sm:flex-row">
                            <Link href="/register" className="group flex h-14 md:h-16 items-center justify-center gap-3 rounded-2xl bg-emerald-600 px-8 md:px-10 text-base md:text-lg font-black text-white shadow-xl shadow-emerald-500/20 transition-all hover:bg-emerald-700 hover:scale-105 active:scale-95">
                                Empezar Gratis
                                <ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" />
                            </Link>

                            <Link href="#" className="flex h-14 md:h-16 items-center justify-center gap-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-8 md:px-10 text-base md:text-lg font-black text-slate-900 dark:text-white transition-all hover:bg-slate-50 dark:hover:bg-slate-800">
                                <Play className="h-4 w-4 fill-current" />
                                Ver Demo
                            </Link>
                        </div>

                        <div className="mt-12 md:mt-16 flex flex-col md:flex-row items-center gap-4 md:gap-6">
                            <div className="flex -space-x-3">
                                {[1, 2, 3, 4].map((i) => (
                                    <div key={i} className="h-10 w-10 rounded-full border-4 border-white dark:border-slate-950 bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-[10px] font-bold text-slate-400 ring-2 ring-emerald-500/10">
                                        JS
                                    </div>
                                ))}
                            </div>
